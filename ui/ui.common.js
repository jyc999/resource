  ...
  render: function render(h) {
    ...
    switch (type) {
      ...
      case 'el-checkbox-group':
        if (!Array.isArray(value)) throw '组件<el-checkbox-group>的value必须是数组';
        sub = config.lists.map(function (item) {
          var id = config.deepGet('props', 'id') || 'id',
              name = config.deepGet('props', 'name') || 'name',
              obj = objectSpread2_objectSpread2({}, item);
...
Object.defineProperty(Object.prototype, 'deepGet', {
  ...
  value: function value() {
    ...
    })) throw new Error('参数类型只能是string、number！');
...
Object.defineProperty(Object.prototype, 'deepSet', {
  ...
  value: function value() {
    ...
    if (args.length < 2) throw new Error('参数个数至少为2！');
    var last1 = args.pop();
    if (args.some(function (item) {
      return ['string', 'number'].indexOf(_typeof(item)) < 0;
    })) throw new Error('除末尾参数外，其余只能是string、number！');
    var last2 = args.pop(),
        obj = args.reduce(function (res, item) {
      if (!res[name]) res[name] = {};
      if (Object.prototype.toString.call(res[name]) != '[object Object]') throw new Error('中间节点格式错误！');
      return res[name];
    }, this);
    obj[last2] = last1;
  }
...
/*
防抖函数
	immediate: true:立即执行，false:wait(ms)之后执行
*/
...
/* harmony default export */ var formDialog_mainvue_type_script_lang_js_ = ({
  ...
  methods: {
    ...
    // 初始化form,若formConf的各项属性在form不存在对应，则进行填充
...
dialog_main.install = function (Vue) {
  Vue.component(dialog_main.name, dialog_main); // 动态渲染弹出框
...
/* harmony default export */ var tabs_mainvue_type_script_lang_js_ = ({
  ...
  methods: {
    ...
    scrollTo: function scrollTo(full_path) {
      ...
      if (!this.$refs.deepGet(full_path, 0)) return; // 若指定标签不在可视区域，则以最小的滚动距离出现在可视区域
